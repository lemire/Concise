...
public:
    ...
    ConciseSet(const ConciseSet& cs) : words(cs.words), last(cs.last), lastWordIndex(cs.lastWordIndex) {}
    ...
    ConciseSet<wah_mode> logicaland(const ConciseSet<wah_mode> & other) const {
        ...
        ConciseSet res;
        res.words.resize(1 + min(
                             (uint32_t)(this->lastWordIndex + other.lastWordIndex + 2),
                             maxLiteralLengthDivision(max(this->last, other.last)) << (wah_mode ? 1 : 0)));
    ...
    bool contains(int32_t o) const {
        if (isEmpty() || o > last || o < 0)
            return false;

        // check if the element is within a literal word
        uint32_t block = maxLiteralLengthDivision(o);
    ...
    void clearBitsAfterInLastWord(int lastSetBit) {
        words[lastWordIndex] &= ALL_ZEROS_LITERAL | (UINT32_C(0xFFFFFFFF) >> (31 - lastSetBit));
    }
    ...
    void appendLiteral(uint32_t word) {
        ...
        if (lastWordIndex == 0 && word == ALL_ZEROS_LITERAL && words[0] == UINT32_C(0x01FFFFFF))
            return;
        ...
        if (word == ALL_ZEROS_LITERAL) {
            ...
            if (lastWord == ALL_ONES_LITERAL)
                ...
                words[lastWordIndex] = SEQUENCE_BIT | 1 | ((1 + __builtin_ctz(~lastWord)) << 25);
            else
                words[++lastWordIndex] = word;
        } else {
            words[++lastWordIndex] = word;
        ...
        if (isLiteral(lastWord)) {
            if (fillType == 0 && lastWord == ALL_ZEROS_LITERAL) {
                ...
                if (fillType == 0 && containsOnlyOneBit(getLiteralBits(lastWord))) {
                    ...
                    words[lastWordIndex] = SEQUENCE_BIT | length | ((1 + __builtin_ctz(~lastWord)) << 25);
                } else {
...
public:
     ...
     * Initialize data
     */
    WordIterator(const ConciseSet<wah_mode> & p) : IsLiteral (false),parent(p),  index(-1), word(0), count(0) {
        prepareNext();
    }
